...
1. preseed.cfg の編集 -> [http-svr/preseed.cfg]  
   とりあえず動きました。何度も検証したかったので http リクエストで取得できるように外に配置。  
   ...
   preseed でのpackage install は最小限にして、後処理で必要なpackageをinstallするようにしています。  
   ...
   と変更して、下記のように設定しています。  
    ...
    wget でget したfileの置き場所も試行錯誤。  
     ...
     (manual に書いてあるのですが、manualを理解できず。正解がわかるとmanualが理解できる感じ)  
    
1. [preseed.cfg]  
   d-i passwd/user-password-crypted password $1$ElgTiXpM$vSun4F.QDb5IyBbGyvXlA0 の部分
   
   MD5 で暗号化して記載しています。(大元は user name と同じ sasaki です)  
   -> [github/userchg.sh] で別途 初期userと同一group に属するuserを作成するshell有。install後に初期Userは破棄)  
...
1. 実行のログ  
  - [vboxmanaage] :  
     virtual box name で作成されたdirectory以下 [Logs]VBox.log,VBoxStartup.log  
  - [ubuntu] :  
     install 後 /var/log/installer/syslog  
